<template>
  <div class="outbound-list-page">
    ...
    <a-drawer
      ...
      :title="detailData?.outboundNo || '出库单详情'"
      placement="right"
      width="90vw"
      ...
      <a-skeleton active :loading="detailLoading" :paragraph="{ rows: 12 }">
        <template v-if="detailData">
          <a-descriptions bordered :column="2" size="small" style="margin-bottom: 16px">
            <a-descriptions-item label="出库单号">{{ detailData.outboundNo }}</a-descriptions-item>
            <a-descriptions-item label="出库类型">{{ detailData.outboundType }}</a-descriptions-item>
            <a-descriptions-item label="仓库">{{ detailData.warehouseName }}</a-descriptions-item>
            <a-descriptions-item label="出库日期">{{ detailData.outboundDate }}</a-descriptions-item>
            ...
            <a-descriptions-item label="目标单号">{{ detailData.targetNo || '-' }}</a-descriptions-item>
    ...
    <a-modal v-model:open="addVisible" title="新建出库单" width="800px" :confirm-loading="addSubmitting"
        ...
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="销售订单" name="orderNo">
              <a-select v-model:value="addForm.orderNo" size="small" show-search placeholder="请选择销售订单"
          ...
          <a-col :span="12">
            <a-form-item label="出库类型" name="outboundType">
              <a-select v-model:value="addForm.outboundType" size="small" placeholder="请选择出库类型" :options="outboundTypeOptions" />
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter="16">
          ...
          <a-col :span="12">
            <a-form-item label="出库日期" name="outboundDate">
              <a-date-picker v-model:value="addForm.outboundDate" size="small" style="width: 100%" placeholder="请选择出库日期" />
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter="16">
      ...
      <VxeTableList :data-source="addForm.items" :pagination="false" row-key="key"
        ...
        <template #availableQtyCell="{ record, rowIndex }">
          <a-tag :color="getAvailableQtyColor(rowIndex)">{{ getAvailableQty(rowIndex) }}</a-tag>
        </template>
        <template #quantityCell="{ record, rowIndex }">
          <a-input-number v-model:value="addForm.items[rowIndex].quantity" :min="1" style="width: 100%" size="small" />
        </template>
        <template #unitPriceCell="{ record, rowIndex }">
          <a-input-number v-model:value="addForm.items[rowIndex].unitPrice" :min="0" :precision="2" style="width: 100%" size="small" />
        </template>
        <template #actionCell="{ record, rowIndex }">
          <a-button type="link" danger size="small" @click="removeOutboundItem(rowIndex)">删除</a-button>
        </template>
      </VxeTableList>
    </a-modal>
  </div>
...
async function fetchDetail(id: number) {
  ...
  try {
    detailData.value = await outboundApi.getById(id) as any
  } catch (err: any) {
    console.warn('[库存出库] 获取详情失败', err)
...
const addItemColumns = [
  ...
  { title: '可用库存', field: 'availableQty', width: 90, slotName: 'availableQtyCell' },
  { title: '出库数量', field: 'quantity', width: 110, slotName: 'quantityCell' },
...
.outbound-list-page > :deep(.vxe-table-list-container) {
  flex: 1;
  min-height: 0;
}
