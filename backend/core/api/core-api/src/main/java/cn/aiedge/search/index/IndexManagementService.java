...
public class IndexManagementService {
        ...
        CreateIndexRequest request = CreateIndexRequest.of(builder ->
                builder.index(indexName)
                        ...
                        .mappings(m -> m
                                ...
                                .properties("totalAmount", p -> p.scaledFloat(sf -> sf.scalingFactor(100.0)))
                                .properties("status", p -> p.keyword(k -> k))
        ...
        CreateIndexRequest request = CreateIndexRequest.of(builder ->
                builder.index(indexName)
                        ...
                        .mappings(m -> m
                                ...
                                .properties("amount", p -> p.scaledFloat(sf -> sf.scalingFactor(100.0)))
                                .properties("status", p -> p.keyword(k -> k))
    ...
    public boolean updateMapping(String indexType, Long tenantId, Map<String, Object> newFields) throws IOException {
        ...
        for (Map.Entry<String, Object> entry : newFields.entrySet()) {
            ...
            properties.put(entry.getKey(), fieldDefinition);
        }
        ...
        // 简单实现：返回false，因为Elasticsearch客户端API使用方式有误
        // 实际项目中需要根据正确的Elasticsearch客户端版本和API进行实现
        log.warn("更新索引映射功能暂未实现: {}", indexName);
        return false;
    }
