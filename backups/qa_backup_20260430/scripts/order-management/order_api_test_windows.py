...
class OrderAPITest:
    ...
    def test_health_check(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"  结果: [PASS] 通过 - 服务状态: {status}")
                self.results.append({'test': '健康检查', 'passed': True})
                return True
            else:
                print(f"  结果: [FAIL] 失败 - HTTP {response.status_code}")
                self.results.append({'test': '健康检查', 'passed': False})
                return False
        except Exception as e:
            print(f"  结果: [FAIL] 失败 - {str(e)}")
    ...
    def test_order_create(self):
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(f"  结果: [PASS] 通过 - 订单ID: {self.created_order_id}")
    ...
    def test_order_get(self):
        ...
        if not self.created_order_id:
            print(f"  结果: [SKIP] 跳过 - 无可用订单ID")
            self.results.append({'test': '订单查询', 'passed': False, 'reason': '跳过'})
            return False
        try:
            response = requests.get(
                f"{self.base_url}/api/v1/order/{self.created_order_id}",
                timeout=10
            )
            if response.status_code == 200:
                order = response.json()
                print(f"  结果: [PASS] 通过 - 数据正确")
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                ...
                if verify_response.status_code == 404:
                    print(f"  结果: [PASS] 通过 - 删除成功")
                    self.results.append({'test': '订单删除', 'passed': True})
                    return True
                else:
                    print(f"  结果: [FAIL] 失败 - 删除后仍可查询")
    ...
    def run_tests(self):
        ...
        report_file = 'order_api_test_report.json'
