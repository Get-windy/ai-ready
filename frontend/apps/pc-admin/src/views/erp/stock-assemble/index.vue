...
import {
  ...
  MinusCircleOutlined, EyeOutlined, BuildOutlined, SearchOutlined
...
// ── 防抖与错误处理 ─────────────────────────────────────
function handleError(err: any) { console.warn('[组装管理] ErrorBoundary 捕获异常:', err) }
...
const filterFields = computed(() => [
  { key: 'keyword', label: '关键字', type: 'input' as const, placeholder: '请输入组装单号/关键字' },
...
const vxeColumns: any = computed(() => [
  { field: 'assembleNo', title: '组装单号', width: 150 },
  { field: 'warehouseName', title: '仓库', width: 120 },
  { field: 'bomNo', title: 'BOM编号', width: 130 },
  { field: 'bomName', title: 'BOM名称', width: 140 },
  { field: 'productCode', title: '成品编码', width: 120 },
  { field: 'productName', title: '成品名称', width: 140 },
  { field: 'assembleQuantity', title: '组装数量', width: 90, align: 'center' },
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const res = await request.get('/erp/stock/assemble/page', {
      params: {
    ...
    hasError.value = true
    console.warn('[组装管理] 获取数据失败', error)
...
const createRules: Record<string, any[]> = {
  ...
  assembleQuantity: [{ required: true, message: '请输入组装数量', trigger: 'blur' }]
}
...
const handleCreateSubmit = async () => {
  ...
  try {
    await request.post('/erp/stock/assemble', {
      bomId: createForm.bomId,
      warehouseId: createForm.warehouseId,
      assembleQuantity: createForm.assembleQuantity,
      assembleFee: createForm.assembleFee || undefined,
...
const fetchDetail = async (id: number) => {
  ...
  detailItems.value = []
  try {
    const res = await request.get(`/erp/stock/assemble/${id}`)
    detailData.value = res.data || null
    try {
      const itemsRes = await request.get(`/erp/stock/assemble/${id}/items`)
      detailItems.value = itemsRes?.data || []
    } catch { detailItems.value = [] }
  } catch (err) {
    console.warn('[组装管理] 获取详情失败', err)
...
const handleSubmitApproval = async (record: any) => {
  Modal.confirm({
    title: '提交审批',
    content: `确认提交组装单 ${record.assembleNo} 进行审批吗？`,
    okText: '确认',
    cancelText: '取消',
    onOk: async () => {
      try {
        await request.post(`/erp/stock/assemble/${record.id}/submit`)
        message.success('提交成功')
        fetchData()
        if (detailVisible.value) detailVisible.value = false
      } catch (error) {
        console.warn('[组装管理] 提交失败', error)
        message.error('提交失败')
      }
    }
  })
...
const handleApprove = async (record: any) => {
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        await request.post(`/erp/stock/assemble/${record.id}/approve`)
        message.success('审批通过')
        fetchData()
        if (detailVisible.value) detailVisible.value = false
      } catch (error) {
        console.warn('[组装管理] 审批失败', error)
        message.error('审批失败')
      }
    }
  })
...
const handleReject = async (record: any) => {
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        await request.post(`/erp/stock/assemble/${record.id}/reject`)
        message.success('已拒绝')
        fetchData()
        if (detailVisible.value) detailVisible.value = false
      } catch (error) {
        console.warn('[组装管理] 拒绝失败', error)
        message.error('拒绝失败')
      }
    }
  })
...
const handleExecute = async (record: any) => {
  Modal.confirm({
    title: '执行确认',
    content: '确认执行该组装单吗？执行后将更新库存。',
    okText: '确认执行',
    cancelText: '取消',
    onOk: async () => {
      try {
        await request.post(`/erp/stock/assemble/${record.id}/execute`)
        message.success('组装执行成功')
        fetchData()
        if (detailVisible.value) detailVisible.value = false
      } catch (error) {
        console.warn('[组装管理] 执行失败', error)
        message.error('组装执行失败')
      }
    }
  })
}
...
const handleCancelConfirm = async () => {
  ...
  try {
    await request.post(`/erp/stock/assemble/${pendingCancelRecord.id}/cancel`, null, {
...
.assemble-header__left {
  display: flex;
  flex-direction: column;
  gap: 2px;
}

.assemble-header__breadcrumb {
  font-size: 12px;
  color: #999;
}

.assemble-header__title {
  font-size: 18px;
  font-weight: 600;
  color: #303133;
  margin: 0;
