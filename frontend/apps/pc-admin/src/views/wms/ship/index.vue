      ...
      <div class="page-header">
        <div class="page-header__left">
          <a-breadcrumb>
            ...
            <a-breadcrumb-item>WMS / 发货管理</a-breadcrumb-item>
          </a-breadcrumb>
          <h2>发货管理</h2>
        </div>
        <div class="page-header__right">
          ...
          <a-button v-permission="'wms:ship:create'" type="primary" size="small" @click="handleAdd"><PlusOutlined /> 新增发货单</a-button>
        ...
        >
          <template #emptyText>
            <div class="empty-state-wrapper">
              ...
              <template v-else>
                ...
                <a-button v-permission="'wms:ship:create'" type="primary" size="small" @click="handleAdd" class="empty-state-action">
                  <PlusOutlined /> 新增发货单
                </a-button>
              </template>
            </div>
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.dataIndex === 'progress'">
              <span>{{ record.shippedQty || 0 }} / {{ record.totalQty || 0 }}</span>
            </template>
            <template v-if="column.dataIndex === 'action'">
              <a-space :size="4">
                <a-tooltip title="查看"><a-button v-permission="'wms:ship:view'" type="link" size="small" @click="handleView(record as any)"><EyeOutlined /></a-button></a-tooltip>
                <a-tooltip title="开始发货" v-if="record.status === 0">
                  <a-button v-permission="'wms:ship:start'" type="link" size="small" @click="handleStart(record as any)"><PlayCircleOutlined /></a-button>
                </a-tooltip>
                <a-tooltip title="确认发货" v-if="record.status === 1">
                  <a-button v-permission="'wms:ship:confirm'" type="link" size="small" @click="handleConfirm(record as any)"><CheckCircleOutlined /></a-button>
                </a-tooltip>
                <a-tooltip title="删除" v-if="record.status === 0">
                  <a-button v-permission="'wms:ship:delete'" type="link" size="small" danger @click="wms.handleDelete(record as any, '确定要删除该发货单吗？')"><DeleteOutlined /></a-button>
                </a-tooltip>
              </a-space>
            </template>
          </template>
      ...
      <a-row :gutter="16">
        ...
        <a-col :span="12">
          <a-form-item label="客户" name="customerName">
            <a-input v-model:value="formState.customerName" size="small" placeholder="请输入客户名称" />
          </a-form-item>
        </a-col>
      </a-row>
      <a-row :gutter="16">
        <a-col :span="12">
          <a-form-item label="承运商" name="carrierName">
            <a-input v-model:value="formState.carrierName" size="small" placeholder="请输入承运商" />
          </a-form-item>
        </a-col>
        <a-col :span="12">
          <a-form-item label="运单号" name="trackingNo">
            <a-input v-model:value="formState.trackingNo" size="small" placeholder="请输入运单号" />
          </a-form-item>
        </a-col>
      </a-row>
      <a-row :gutter="16">
...
import { shipApi, type WmsShipTask } from '@/api/wms/ship'
...
function handleError(err: any) { console.warn('[WMS发货]', err) }
const hasError = ref(false)

const statusMap: Record<number, { text: string; color: string }> = {
  0: { text: '待处理', color: 'orange' },
  1: { text: '发货中', color: 'blue' },
...
const columns = [
  ...
  { title: '运单号', dataIndex: 'trackingNo', width: 130 },
...
const searchFields: SearchField[] = [
  ...
  { name: 'sourceNo', label: '来源单号', type: 'input', placeholder: '请输入来源单号' },
  { name: 'status', label: '状态', type: 'select', placeholder: '请选择状态', options: [
    { label: '待处理', value: 0 }, { label: '发货中', value: 1 }, { label: '已完成', value: 2 }, { label: '已取消', value: 3 }, { label: '异常', value: 4 },
  ]},
]

const wms = useWmsTable({
  fetchFn: shipApi.page as any,
  deleteFn: shipApi.remove,
  defaultPageSize: 20,
  refreshInterval: 30,
...
const modalTitle = computed(() => (isEdit.value ? '编辑发货单' : '新增发货单'))
const formRef = ref<FormInstance>()
const formState = reactive<Record<string, any>>({
  id: 0, taskNo: '', sourceNo: '', warehouseName: '', customerName: '',
  carrierName: '', trackingNo: '', totalQty: 0, remark: '',
})
...
function handleEdit(record: WmsShipTask) {
  isEdit.value = true
...
async function handleModalOk() {
  ...
  try {
    ...
    if (isEdit.value) {
      await shipApi.update(payload)
      message.success('更新成功')
    } else {
      await shipApi.save(payload)
...
function handleView(record: WmsShipTask) {
  message.info(`查看发货单: ${record.taskNo}`)
}

// ── 工作流操作 ──
async function handleStart(record: WmsShipTask) {
  try {
    await shipApi.startShip(record.id)
    message.success('发货已开始')
    wms.fetchData()
  } catch (err: any) {
...
async function handleConfirm(record: WmsShipTask) {
  Modal.confirm({
    title: '确认发货',
    content: `确定要确认发货单"${record.taskNo}"吗？`,
    onOk: async () => {
      try {
        await shipApi.confirmShip(record.id)
