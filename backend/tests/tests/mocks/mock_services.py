...
class MockPermissionService:
    ...
    def __init__(self):
        self.roles = {
            ...
            'USER': {'permissions': ['user:read', 'user:update:self', 'order:read', 'inventory:read'], 'menus': [1]},
            'ERP_ADMIN': {'permissions': ['erp:*'], 'menus': [4,5]},
            'CRM_ADMIN': {'permissions': ['crm:*'], 'menus': [6,7]},
            'SALES': {'permissions': ['order:read', 'order:create', 'order:update', 'inventory:read', 'inventory:lock'], 'menus': [1,4]},
            'WAREHOUSE': {'permissions': ['inventory:read', 'inventory:check'], 'menus': [4]},
            'WAREHOUSE_ADMIN': {'permissions': ['inventory:read', 'inventory:write', 'inventory:inbound', 'inventory:outbound', 'inventory:adjust'], 'menus': [4,5]},
            'FINANCE': {'permissions': ['inventory:read', 'inventory:adjust', 'order:read', 'order:approve'], 'menus': [4,6]},
            'MANAGER': {'permissions': ['order:read', 'order:create', 'order:approve', 'inventory:read', 'inventory:adjust', 'user:read'], 'menus': [1,2,4]}
        }
        self.user_roles = {}
...
class MockERPService:
    ...
    def create_shopping_cart(self, cart_data: Dict) -> Dict:
        """创建购物车"""
        cart_id = len(self.shopping_carts) + 1
        cart_data['id'] = cart_id
        self.shopping_carts[cart_id] = cart_data
        return cart_data
    
    def clear_shopping_cart(self, cart_id: int):
        """清空购物车"""
        if cart_id in self.shopping_carts:
...
class MockPaymentService:
    ...
    def create_payment(self, payment_data: Dict) -> Dict:
        ...
        payment_data['create_time'] = datetime.now()
        self.payments[payment_id] = payment_data
        return payment_data
    
    def process_payment(self, payment_id: int, success: bool = True) -> Dict:
        """处理支付"""
        ...
        if success:
            ...
            payment['complete_time'] = datetime.now()
            ...
            payment['fail_time'] = datetime.now()
    ...
    def refund_payment(self, payment_id: int, amount: float, reason: str = '') -> Dict:
        ...
        if payment['status'] != 'success':
            return {'success': False, 'message': '只有成功支付才能退款'}
        
        if amount > payment['amount']:
            return {'success': False, 'message': '退款金额不能超过支付金额'}
