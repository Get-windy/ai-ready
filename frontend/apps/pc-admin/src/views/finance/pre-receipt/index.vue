        ...
        <template #action="{ record }">
          <a-space :size="0" class="action-cell-inner">
            ...
            <a-tooltip :title="record.status !== 'received' ? '' : '转收款'">
              <a-button type="link" size="small" :disabled="record.status !== 'received'" @click="handleOffsetToReceipt(record)">
                <template #icon><SwapOutlined /></template>
              </a-button>
            </a-tooltip>
            <a-tooltip :title="record.status !== 'received' ? '' : '没收'">
              <a-button type="link" size="small" :disabled="record.status !== 'received'" @click="handleForfeit(record)">
                <template #icon><StopOutlined /></template>
              </a-button>
            </a-tooltip>
            <a-tooltip :title="record.status !== 'received' ? '' : '退款'">
              <a-button type="link" size="small" :disabled="record.status !== 'received'" @click="handleRefund(record)">
