<template>
  ...
  >
    ...
    <template #empty>
      <a-empty v-if="hasActiveFilters" description="当前筛选条件下无匹配付款单">
        <template #image><SearchOutlined style="font-size: 48px; color: #faad14" /></template>
        <a-button @click="handleResetFilters">清除筛选</a-button>
      </a-empty>
      <a-empty v-else description="暂无付款单">
        <template #image><InboxOutlined style="font-size: 48px; color: #d9d9d9" /></template>
        <a-button type="primary" @click="handleAdd">新建付款单</a-button>
      </a-empty>
    </template>
...
import { PlusOutlined, EyeOutlined, DeleteOutlined, CheckCircleOutlined, PrinterOutlined, SearchOutlined, InboxOutlined, EllipsisOutlined } from '@ant-design/icons-vue'
...
function handleExport() {
  executeExport({
    fileName: '付款单',
    headers: ['付款单号', '采购订单', '供应商', '付款日期', '付款金额', '付款方式', '状态', '创建时间'],
...
.print-header { margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center; }
