<template>
      ...
      <div class="table-empty">
        ...
        <template v-else>
          ...
          <p v-if="hasActiveFilters" class="table-empty-text">
            当前筛选条件下无匹配退货单，<a @click="handleResetFilters">清除筛选</a>
          </p>
    ...
    <template #action="{ record }">
      <a-space :size="0" class="action-cell-inner">
        <a-tooltip title="查看详情">
		  ...
		  <PrintButton :record="record" :business-id="record.id" business-type="sale_return" button-type="link" button-size="small" tooltip="打印" />
  ...
  <a-modal v-model:open="formModalVisible" title="新建退货单" width="800px" centered
    ...
    <a-form ref="formRef" :model="formData" :rules="formRules" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">
      <a-form-item label="销售订单" name="orderNo"><a-input v-model:value="formData.orderNo" placeholder="请输入销售订单号" size="small" /></a-form-item>
      ...
      <a-row>
        <a-col :span="12">
          <a-form-item label="退货原因" name="reason" :label-col="{ span: 10 }" :wrapper-col="{ span: 14 }">
            <a-select v-model:value="formData.reason" placeholder="请选择退货原因" size="small">
        ...
        <a-col :span="12">
          <a-form-item label="退货日期" name="returnDate" :label-col="{ span: 10 }" :wrapper-col="{ span: 14 }">
            <a-date-picker v-model:value="formData.returnDate" size="small" style="width: 100%" /></a-form-item>
        </a-col>
      </a-row>
...
import { PlusOutlined, EyeOutlined, DeleteOutlined, CheckCircleOutlined, InboxOutlined, SearchOutlined, EllipsisOutlined, FileOutlined, ClockCircleOutlined, RollbackOutlined, WarningOutlined, ReloadOutlined } from '@ant-design/icons-vue'
...
.sale-return-page > :deep(.vxe-table-list-container) {
  flex: 1;
  min-height: 0;
}
