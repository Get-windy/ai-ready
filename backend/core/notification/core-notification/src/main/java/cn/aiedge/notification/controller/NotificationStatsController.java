...
public class NotificationStatsController {
    ...
    public ResponseEntity<SendSummary> getSendSummary() {
        // 统计各状态数量
        int pending = recordMapper.findByStatus(NotificationRecord.STATUS_PENDING).size();
        int sending = recordMapper.findByStatus(NotificationRecord.STATUS_SENDING).size();
        int success = recordMapper.findByStatus(NotificationRecord.STATUS_SUCCESS).size();
        int failed = recordMapper.findByStatus(NotificationRecord.STATUS_FAILED).size();
    ...
    public ResponseEntity<RetryResult> retryFailed() {
        List<NotificationRecord> failedRecords = 
            recordMapper.findByStatus(NotificationRecord.STATUS_FAILED);
