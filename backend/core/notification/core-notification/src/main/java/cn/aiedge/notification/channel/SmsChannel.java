...
public class SmsChannel implements NotificationChannel {
    ...
    // 例如：private final SmsClient smsClient;

    @Override
    public SendResult send(NotificationRecord record) {
        if (!isAvailable()) {
            return SendResult.fail("短信服务未启用");
        }
        ...
        try {
            ...
            // 模拟发送成功
            // 实际项目中取消注释以下代码：String externalId = smsClient.send(phone, record.getContent());
            ...
            return SendResult.fail("短信发送失败: " + e.getMessage());
        }
    }
