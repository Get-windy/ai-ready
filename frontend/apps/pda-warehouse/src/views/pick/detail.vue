...
interface PickItem {
  ...
  status: 'pending' | 'picked' | 'exception'
}
...
const handleConfirmPick = async () => {
  ...
  try {
    ...
    showToast({ type: 'success', message: '拣货成功' })
    
    const nextItem = items.value.find(i => i.status === 'pending')
    if (nextItem) {
      currentItem.value = nextItem
    } else {
      showToast({ type: 'success', message: '拣货完成' })
      router.push('/pick')
    }
  } finally {
    closeToast()
...
const getProgress = () => {
  const picked = items.value.filter(i => i.status === 'picked').length
  ...
  <div class="pick-detail-page">
    ...
    <div class="task-info">
      ...
      <div class="progress-info">
        ...
        <span>{{ items.filter(i => i.status === 'picked').length }}/{{ items.length }}</span>
      </div>
    </div>
        ...
        <template #title>
          <div class="item-header">
            ...
            <Tag :color="currentItem.status === 'pending' ? '#1988fa' : '#07c160'">
              {{ currentItem.status === 'pending' ? '待拣' : '已拣' }}
            </Tag>
          </div>
        </template>
    ...
    <div class="item-list">
      ...
      <div 
        ...
        :key="item.id"
        class="list-item"
        :class="{ completed: item.status === 'picked' }"
