<template>
      ...
      <div class="page-header">
        <div class="page-header__left">
          <a-breadcrumb>
            ...
            <a-breadcrumb-item>WMS / 上架管理</a-breadcrumb-item>
          </a-breadcrumb>
          <h2>上架管理</h2>
        </div>
        <div class="page-header__right">
          ...
          <a-button v-permission="'wms:putaway:create'" type="primary" size="small" @click="handleAdd"><PlusOutlined /> 新增上架单</a-button>
        ...
        >
          <template #emptyText>
            <div class="empty-state-wrapper">
              ...
              <template v-else>
                ...
                <a-button v-permission="'wms:putaway:create'" type="primary" size="small" @click="handleAdd" class="empty-state-action">
                  <PlusOutlined /> 新增上架单
                </a-button>
              </template>
            </div>
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.dataIndex === 'progress'">
              <span>{{ record.putawayQty || 0 }} / {{ record.totalQty || 0 }}</span>
            </template>
            <template v-if="column.dataIndex === 'action'">
              <a-space :size="4">
                <a-tooltip title="查看"><a-button v-permission="'wms:putaway:view'" type="link" size="small" @click="handleView(record as any)"><EyeOutlined /></a-button></a-tooltip>
                <a-tooltip title="开始上架" v-if="record.status === 0">
                  <a-button v-permission="'wms:putaway:start'" type="link" size="small" @click="handleStart(record as any)"><PlayCircleOutlined /></a-button>
                </a-tooltip>
                <a-tooltip title="确认上架" v-if="record.status === 1">
                  <a-button v-permission="'wms:putaway:confirm'" type="link" size="small" @click="handleConfirm(record as any)"><CheckCircleOutlined /></a-button>
                </a-tooltip>
                <a-tooltip title="删除" v-if="record.status === 0">
                  <a-button v-permission="'wms:putaway:delete'" type="link" size="small" danger @click="wms.handleDelete(record as any, '确定要删除该上架单吗？')"><DeleteOutlined /></a-button>
                </a-tooltip>
              </a-space>
            </template>
          </template>
  ...
  >
    <a-form ref="formRef" :model="formState" :rules="formRules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
      ...
      <a-row :gutter="16">
        <a-col :span="12">
          <a-form-item label="目标货位" name="locationCode">
...
import { putawayApi, type WmsPutawayTask } from '@/api/wms/putaway'
...
function handleError(err: any) { console.warn('[WMS上架]', err) }
const hasError = ref(false)

const statusMap: Record<number, { text: string; color: string }> = {
  0: { text: '待处理', color: 'orange' },
  1: { text: '上架中', color: 'blue' },
...
const columns = [
  ...
  { title: '目标货位', dataIndex: 'locationCode', width: 120 },
...
const searchFields: SearchField[] = [
  ...
  { name: 'status', label: '状态', type: 'select', placeholder: '请选择状态', options: [
    { label: '待处理', value: 0 }, { label: '上架中', value: 1 }, { label: '已完成', value: 2 }, { label: '已取消', value: 3 }, { label: '异常', value: 4 },
  ]},
]

const wms = useWmsTable({
  fetchFn: putawayApi.page as any,
  deleteFn: putawayApi.remove,
  defaultPageSize: 20,
  refreshInterval: 30,
...
const modalTitle = computed(() => (isEdit.value ? '编辑上架单' : '新增上架单'))
const formRef = ref<FormInstance>()
const formState = reactive<Record<string, any>>({
  id: 0, sourceNo: '', warehouseName: '', locationCode: '', totalQty: 0, remark: '',
})
...
function handleAdd() {
  ...
  Object.assign(formState, { id: 0, sourceNo: '', warehouseName: '', locationCode: '', totalQty: 0, remark: '' })
  modalVisible.value = true
}

function handleEdit(record: WmsPutawayTask) {
  isEdit.value = true
...
async function handleModalOk() {
  ...
  try {
    ...
    if (isEdit.value) {
      await putawayApi.update(payload)
      message.success('更新成功')
    } else {
      await putawayApi.save(payload)
...
function handleView(record: WmsPutawayTask) {
  message.info(`查看上架单: ${record.taskNo}`)
}

// ── 工作流操作 ──
async function handleStart(record: WmsPutawayTask) {
  try {
    await putawayApi.startPutaway(record.id)
    message.success('上架已开始')
    wms.fetchData()
  } catch (err: any) {
...
async function handleConfirm(record: WmsPutawayTask) {
  Modal.confirm({
    title: '确认上架',
    content: `确定要确认上架单"${record.taskNo}"吗？`,
    onOk: async () => {
      try {
        await putawayApi.confirmPutaway(record.id)
        message.success('上架确认成功')
        wms.fetchData()
      } catch (err: any) {
        message.error(err?.message || '操作失败')
      }
