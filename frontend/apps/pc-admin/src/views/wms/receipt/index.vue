...
import { receiptApi, type WmsReceiptTask } from '@/api/wms/receipt'
...
function handleError(err: any) { console.warn('[WMS收货]', err) }
const hasError = ref(false)

const statusMap: Record<number, { text: string; color: string }> = {
  0: { text: '待处理', color: 'orange' },
  1: { text: '收货中', color: 'blue' },
...
const sourceTypeMap: Record<number, string> = {
  0: '采购入库', 1: '生产入库', 2: '退货入库', 3: '调拨入库', 4: '其他',
}
...
const columns = [
  ...
  { title: '供应商', dataIndex: 'supplierName', width: 130, ellipsis: true },
...
const searchFields: SearchField[] = [
  ...
  { name: 'status', label: '状态', type: 'select', placeholder: '请选择状态', options: [
    { label: '待处理', value: 0 }, { label: '收货中', value: 1 }, { label: '已完成', value: 2 }, { label: '已取消', value: 3 }, { label: '异常', value: 4 },
  ]},
]

const wms = useWmsTable({
  fetchFn: receiptApi.page as any,
  deleteFn: receiptApi.remove,
  defaultPageSize: 20,
  refreshInterval: 30,
...
const modalTitle = computed(() => (isEdit.value ? '编辑收货单' : '新增收货单'))
const formRef = ref<FormInstance>()
const formState = reactive<Record<string, any>>({
  id: 0, taskNo: '', sourceType: 0, sourceNo: '', warehouseName: '', supplierName: '', expectedQty: 0, remark: '',
})
...
function handleAdd() {
  ...
  Object.assign(formState, { id: 0, taskNo: '', sourceType: 0, sourceNo: '', warehouseName: '', supplierName: '', expectedQty: 0, remark: '' })
  modalVisible.value = true
}

function handleEdit(record: WmsReceiptTask) {
  isEdit.value = true
...
async function handleModalOk() {
  ...
  try {
    ...
    if (isEdit.value) {
      await receiptApi.update(payload)
...
function handleView(record: WmsReceiptTask) {
  message.info(`查看收货单: ${record.taskNo}`)
}

// ── 工作流操作 ──
async function handleStart(record: WmsReceiptTask) {
  try {
    await receiptApi.startReceipt(record.id)
    message.success('收货已开始')
    wms.fetchData()
  } catch (err: any) {
...
async function handleConfirm(record: WmsReceiptTask) {
  Modal.confirm({
    title: '确认收货',
    content: `确定要确认收货单"${record.taskNo}"吗？`,
    onOk: async () => {
      try {
        await receiptApi.confirmReceipt(record.id)
...
async function handleCancel(record: WmsReceiptTask) {
  Modal.confirm({
    title: '取消收货',
    content: `确定要取消收货单"${record.taskNo}"吗？`,
    onOk: async () => {
      try {
        await receiptApi.cancelReceipt(record.id)
        message.success('已取消')
        wms.fetchData()
      } catch (err: any) {
        message.error(err?.message || '操作失败')
