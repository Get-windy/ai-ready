...
const loadDashboard = async () => {
  ...
  try {
    ...
    if (res.data) {
      dashboardData.totalAssets = res.totalAssets || 0
      dashboardData.totalLiabilities = res.totalLiabilities || 0
      dashboardData.monthlyIncome = res.monthlyIncome || 0
      dashboardData.monthlyExpense = res.monthlyExpense || 0
      dashboardData.netProfit = (res.monthlyIncome || 0) - (res.monthlyExpense || 0)

      if (res.todoCounts) {
        todoCounts.unaudited = res.todoCounts.unaudited || 0
        todoCounts.unposted = res.todoCounts.unposted || 0
        todoCounts.overdueReceivable = res.todoCounts.overdueReceivable || 0
        todoCounts.overduePayable = res.todoCounts.overduePayable || 0
      }
    }
