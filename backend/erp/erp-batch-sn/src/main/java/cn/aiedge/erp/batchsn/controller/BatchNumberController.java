...
import cn.aiedge.erp.batchsn.controller.response.BatchApiResponse;
...
public class BatchNumberController {
    ...
    @Operation(summary = "创建批次", description = "创建新的批次记录，支持自动生成批次号")
    @ApiResponses({
        @ApiResponse(responseCode = "201", description = "批次创建成功", 
            content = @Content(schema = @Schema(implementation = BatchApiResponse.class))),
    ...
    public ResponseEntity<BatchApiResponse<BatchNumber>> getBatch(@PathVariable @NotNull Long id) {
        ...
        if (cachedBatch != null) {
            ...
            return ResponseEntity.ok(BatchApiResponse.success(cachedBatch));
        }
        BatchNumber batch = batchNumberService.getBatchById(id);
        if (batch == null) {
            log.warn("批次未找到: ID={}", id);
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(BatchApiResponse.notFound("批次未找到"));
        }
    ...
    public ResponseEntity<BatchApiResponse<List<BatchNumber>>> listBatches(@Valid BatchQueryRequest request) {
        ...
        if (cachedBatches != null) {
            ...
            return ResponseEntity.ok(BatchApiResponse.success(cachedBatches));
        }
    ...
    public ResponseEntity<BatchApiResponse<Integer>> updateBatchStatus(
        ...
        if (batchIds.isEmpty()) {
            return ResponseEntity.badRequest().body(BatchApiResponse.error("批次ID列表不能为空"));
        }
    ...
    @Operation(summary = "批次入库", description = "执行批次入库操作")
    @ApiResponses({
    ...
    @Operation(summary = "批次出库", description = "执行批次出库操作")
    @ApiResponses({
    ...
    public ResponseEntity<BatchApiResponse<BatchNumber>> outbound(
        ...
        if (quantity.compareTo(BigDecimal.ZERO) <= 0) {
            return ResponseEntity.badRequest().body(BatchApiResponse.error("出库数量必须大于0"));
        }
        ...
        if (result == null) {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(BatchApiResponse.notFound("批次未找到或库存不足"));
        }
    ...
    public ResponseEntity<BatchApiResponse<String>> clearCache() {
        ...
        log.info("批次缓存已全部清除");
        return ResponseEntity.ok(BatchApiResponse.success("缓存已清除"));
    }
    
    @Operation(summary = "批次转移", description = "执行批次在不同仓库/库位间的转移操作")
    @ApiResponses({
    ...
    public ResponseEntity<BatchApiResponse<BatchNumber>> transferBatch(@Valid @RequestBody BatchTransferRequest request) {
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(BatchApiResponse.error(e.getMessage()));
        }
        BatchNumber result = batchNumberService.transfer(
            request.getBatchId(), request.getFromWarehouseId(), request.getToWarehouseId(),
            request.getFromLocationId(), request.getToLocationId(), request.getQuantity(), request.getRemark()
        );
        if (result == null) {
    ...
    @Operation(summary = "批次盘点", description = "执行批次盘点操作")
    @ApiResponses({
    ...
    public ResponseEntity<BatchApiResponse<BatchNumber>> inventoryBatch(@Valid @RequestBody BatchInventoryRequest request) {
        ...
        BatchNumber result = batchNumberService.inventory(
            request.getBatchId(), request.getPhysicalQuantity(), request.getAdjustmentQuantity(),
            request.getAdjustmentReason(), request.getOperatorId(), request.getOperatorName()
        );
        if (result == null) {
        ...
        cacheManager.evictBatchCache(request.getBatchId());
        cacheManager.clearAllListCache();
        log.info("批次盘点完成: batchId={}, physicalQty={}", request.getBatchId(), request.getPhysicalQuantity());
        return ResponseEntity.ok(BatchApiResponse.success(result));
    }
    
    @Operation(summary = "批次高级搜索", description = "支持多条件复杂查询")
    @ApiResponses({
    ...
    public ResponseEntity<BatchApiResponse<List<BatchNumber>>> advancedSearch(@Valid @RequestBody BatchAdvancedSearchRequest request) {
        ...
        List<BatchNumber> batches = batchNumberService.searchBatches(
            request.getBatchNo(), request.getProductCode(), request.getProductName(),
            request.getStatus(), request.getQualityStatus(), request.getSourceType(),
            request.getProductionDateStart(), request.getProductionDateEnd(),
            request.getExpirationDateStart(), request.getExpirationDateEnd(),
            request.getWarehouseId(), request.getWarehouseName(),
