...
const filterFields = [
  { key: 'keyword', label: '关键词', type: 'input' as const, placeholder: '付款单号/供应商/来源单号' },
  ...
  { key: 'sourceType', label: '来源类型', type: 'select' as const, options: [
    { label: '全部', value: undefined },
    { label: '采购付款', value: 'PURCHASE_ORDER' },
    { label: '销售退货退款', value: 'SALE_RETURN' },
    { label: '预付转正', value: 'PRE_PAYMENT' },
    { label: '定金支付', value: 'DEPOSIT' },
    { label: '费用报销', value: 'EXPENSE' },
    { label: '资产购置', value: 'ASSET_PURCHASE' },
...
const statusColorMap: Record<number, string> = {
  [-1]: 'error', 0: 'default', 1: 'processing', 2: 'success', 3: 'warning', 4: 'success', 5: 'success'
}

const statusLabelMap: Record<number, string> = {
  [-1]: '已取消', 0: '草稿', 1: '待审批', 2: '已审批', 3: '核销中', 4: '已核销', 5: '已完成'
}
...
const columns = computed(() => [
  ...
  { title: '金额', field: 'paymentAmount', width: 120, align: 'right', formatter: ({ cellValue }: any) => cellValue ? `¥${formatAmount(cellValue)}` : '-' },
  ...
  { title: '来源', field: 'sourceType', width: 110, align: 'center', slotName: 'paymentTypeCell' },
  ...
  try {
    ...
    if (res.data) {
      ...
      if ((res.data as any).totalAmount !== undefined) {
        ...
        stats.totalAmount = list.reduce((s: number, i: any) => s + (i.paymentAmount || 0), 0)
    ...
    console.warn('[付款单] 获取数据失败', err)
    message.error('获取付款单数据失败')
    tableData.value = []
    pagination.total = 0
    hasError.value = true
  } finally {
...
const handleDetailClose = () => { detailVisible.value = false; detailData.value = null }
...
const handleAdd = () => {
  ...
  Object.assign(formState, { supplierName: '', paymentAmount: undefined, paymentMethod: 'bank_transfer', sourceType: 'PURCHASE_ORDER', paymentDate: undefined, bankAccount: '', remark: '' })
...
const handleEdit = (record: any) => {
  ...
  Object.assign(formState, {
    id: record.id, supplierName: record.supplierName, paymentAmount: record.paymentAmount,
    paymentMethod: record.paymentMethod || 'bank_transfer', sourceType: record.sourceType || 'PURCHASE_ORDER',
    paymentDate: record.paymentDate ? dayjs(record.paymentDate) : undefined, bankAccount: record.bankAccount || '', remark: record.remark || ''
  })
  initialFormSnapshot.value = JSON.stringify({ ...formState })
  formVisible.value = true
}
...
const handleFormSave = async () => {
  try {
    ...
    if (data.paymentDate && dayjs.isDayjs(data.paymentDate)) data.paymentDate = data.paymentDate.format('YYYY-MM-DD')
    if (isEditing.value) {
      await paymentApi.update(data.id, data)
    ...
    console.warn('[付款单] 保存失败', err)
  } finally { formLoading.value = false }
}

const handleFormClose = () => { formVisible.value = false; formRef.value?.resetFields() }

const handleSubmit = (record: any) => {
  Modal.confirm({ title: '确认提交', content: `确定提交付款单 "${record.paymentNo}" 审批吗？`, async onOk() { await paymentApi.submit(record.id); message.success('已提交审批'); fetchData() } })
}

const handleApprove = (record: any) => {
  Modal.confirm({ title: '确认审批通过', content: `确定审批通过付款单 "${record.paymentNo}" 吗？`, async onOk() { await paymentApi.approve(record.id); message.success('审批通过'); fetchData() } })
}

const handleComplete = (record: any) => {
  Modal.confirm({ title: '确认完成', content: `确定完成付款单 "${record.paymentNo}" 吗？`, async onOk() { await paymentApi.complete(record.id); message.success('付款已完成'); fetchData() } })
}

const handleCancel = (record: any) => { cancelTarget.value = record; cancelReason.value = ''; cancelVisible.value = true }

const handleCancelConfirm = async () => {
  if (!cancelReason.value.trim()) { message.warning('请输入取消原因'); return }
  cancelLoading.value = true
  try {
    await paymentApi.cancel(cancelTarget.value.id, cancelReason.value)
    message.success('付款单已取消'); cancelVisible.value = false; fetchData()
  } catch (err) { console.warn('[付款单] 取消失败', err); message.error('取消失败') }
...
<style scoped>
.pmt-page-header { display: flex; justify-content: space-between; align-items: center; width: 100%; }
.pmt-page-header-left { display: flex; align-items: center; gap: 12px; }
.pmt-page-header-title { font-size: 18px; font-weight: 600; color: #303133; margin: 0; }
.pmt-page-header-right { display: flex; align-items: center; gap: 12px; }
...
.finance-pmt-page { padding: 16px; height: 100%; display: flex; flex-direction: column; overflow: hidden; min-height: 0; gap: 16px; }
.finance-pmt-page > :deep(.vxe-table-list-container) { flex: 1; min-height: 0; }
