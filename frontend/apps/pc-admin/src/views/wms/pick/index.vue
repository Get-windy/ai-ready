      ...
      <div class="page-header">
        <div class="page-header__left">
          <a-breadcrumb>
            ...
            <a-breadcrumb-item>WMS / 拣货管理</a-breadcrumb-item>
          </a-breadcrumb>
          <h2>拣货管理</h2>
        </div>
        <div class="page-header__right">
          ...
          <a-button v-permission="'wms:pick:create'" type="primary" size="small" @click="handleAdd"><PlusOutlined /> 新增拣货任务</a-button>
        ...
        >
          <template #emptyText>
            <div class="empty-state-wrapper">
              ...
              <template v-else>
                ...
                <a-button v-permission="'wms:pick:create'" type="primary" size="small" @click="handleAdd" class="empty-state-action">
                  <PlusOutlined /> 新增拣货任务
                </a-button>
              </template>
            </div>
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.dataIndex === 'action'">
              <a-space :size="4">
                <a-tooltip title="查看"><a-button v-permission="'wms:pick:view'" type="link" size="small" @click="handleView(record as any)"><EyeOutlined /></a-button></a-tooltip>
                <a-tooltip title="开始拣货" v-if="record.status === 0">
                  <a-button v-permission="'wms:pick:start'" type="link" size="small" @click="handleStart(record as any)"><PlayCircleOutlined /></a-button>
                </a-tooltip>
                <a-tooltip title="完成拣货" v-if="record.status === 1">
                  <a-button v-permission="'wms:pick:complete'" type="link" size="small" @click="handleComplete(record as any)"><CheckCircleOutlined /></a-button>
                </a-tooltip>
                <a-tooltip title="删除" v-if="record.status === 0">
                  <a-button v-permission="'wms:pick:delete'" type="link" size="small" danger @click="wms.handleDelete(record as any, '确定要删除该拣货任务吗？')"><DeleteOutlined /></a-button>
                </a-tooltip>
              </a-space>
            </template>
          </template>
      ...
      <a-row :gutter="16">
        <a-col :span="12">
          <a-form-item label="波次编号" name="waveNo">
            <a-input v-model:value="formState.waveNo" size="small" placeholder="请输入波次编号" />
          </a-form-item>
        </a-col>
        <a-col :span="12">
...
import { pickApi, type WmsPickTask } from '@/api/wms/pick'
...
function handleError(err: any) { console.warn('[WMS拣货]', err) }
...
const columns = [
  ...
  { title: '波次编号', dataIndex: 'waveNo', width: 140 },
...
const searchFields: SearchField[] = [
  ...
  { name: 'status', label: '状态', type: 'select', placeholder: '请选择状态', options: [
    { label: '待处理', value: 0 }, { label: '拣货中', value: 1 }, { label: '已完成', value: 2 }, { label: '已取消', value: 3 }, { label: '异常', value: 4 },
  ]},
]

const wms = useWmsTable({
  fetchFn: pickApi.taskPage as any,
  deleteFn: pickApi.taskRemove,
  defaultPageSize: 20,
  refreshInterval: 30,
...
const modalTitle = computed(() => (isEdit.value ? '编辑拣货任务' : '新增拣货任务'))
const formRef = ref<FormInstance>()
const formState = reactive<Record<string, any>>({
  id: 0, waveNo: '', warehouseName: '', sourceNo: '', totalQty: 0, remark: '',
})
...
function handleAdd() {
  ...
  Object.assign(formState, { id: 0, waveNo: '', warehouseName: '', sourceNo: '', totalQty: 0, remark: '' })
  modalVisible.value = true
}

function handleEdit(record: WmsPickTask) {
  isEdit.value = true
...
async function handleModalOk() {
  ...
  try {
    ...
    if (isEdit.value) {
      await pickApi.taskUpdate(payload)
      message.success('更新成功')
    } else {
      await pickApi.taskSave(payload)
...
function handleView(record: WmsPickTask) {
  message.info(`查看拣货任务: ${record.taskNo}`)
}

// ── 工作流操作 ──
async function handleStart(record: WmsPickTask) {
  try {
    await pickApi.taskStart(record.id)
...
async function handleComplete(record: WmsPickTask) {
  Modal.confirm({
    title: '完成拣货',
    content: `确定要完成拣货任务"${record.taskNo}"吗？`,
    onOk: async () => {
      try {
