<template>
      ...
      <div class="page-header">
        <div class="page-header__left">
          <a-breadcrumb>
            ...
            <a-breadcrumb-item>WMS / 波次管理</a-breadcrumb-item>
          </a-breadcrumb>
          <h2>波次管理</h2>
        </div>
        <div class="page-header__right">
          ...
          <a-button v-permission="'wms:wave:create'" type="primary" size="small" @click="handleAdd"><PlusOutlined /> 新增波次</a-button>
      ...
      <div class="page-body">
        ...
        >
          <template #emptyText>
            <div class="empty-state-wrapper">
              ...
              <template v-else>
                ...
                <a-button v-permission="'wms:wave:create'" type="primary" size="small" @click="handleAdd" class="empty-state-action">
                  <PlusOutlined /> 新增波次
                </a-button>
              </template>
            </div>
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.dataIndex === 'waveType'">
              <a-tag>{{ waveTypeMap[record.waveType] || record.waveType }}</a-tag>
            </template>
            <template v-if="column.dataIndex === 'progress'">
              <span>{{ record.pickedQty || 0 }} / {{ record.totalQty || 0 }}</span>
            </template>
            <template v-if="column.dataIndex === 'action'">
              <a-space :size="4">
                <a-tooltip title="查看"><a-button v-permission="'wms:wave:view'" type="link" size="small" @click="handleView(record as any)"><EyeOutlined /></a-button></a-tooltip>
                <a-tooltip title="编辑"><a-button v-permission="'wms:wave:edit'" type="link" size="small" @click="handleEdit(record as any)"><EditOutlined /></a-button></a-tooltip>
                <a-tooltip title="生成拣货任务" v-if="record.status === 0">
                  <a-button v-permission="'wms:wave:create-tasks'" type="link" size="small" @click="handleCreateTasks(record as any)"><PartitionOutlined /></a-button>
                </a-tooltip>
                <a-tooltip title="删除" v-if="record.status === 0">
                  <a-button v-permission="'wms:wave:delete'" type="link" size="small" danger @click="wms.handleDelete(record as any, '确定要删除该波次吗？')"><DeleteOutlined /></a-button>
                </a-tooltip>
              </a-space>
            </template>
          </template>
  ...
  >
    <a-form ref="formRef" :model="formState" :rules="formRules" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
      <a-row :gutter="16">
        <a-col :span="12">
          <a-form-item label="波次编号" name="waveNo">
            <a-input v-model:value="formState.waveNo" size="small" placeholder="系统自动生成" disabled />
          </a-form-item>
        </a-col>
        <a-col :span="12">
      ...
      <a-row :gutter="16">
        <a-col :span="12">
          <a-form-item label="波次类型" name="waveType">
            <a-select v-model:value="formState.waveType" size="small" placeholder="请选择波次类型">
              <a-select-option :value="0">按单拣货</a-select-option>
              <a-select-option :value="1">批量拣货</a-select-option>
              <a-select-option :value="2">按路线</a-select-option>
            </a-select>
          </a-form-item>
        </a-col>
        <a-col :span="12">
...
import { pickApi, type WmsPickWave } from '@/api/wms/pick'
import { useWmsTable } from '@/composables/useWmsTable'
import {
  PlusOutlined, ReloadOutlined, EyeOutlined, EditOutlined, DeleteOutlined, SyncOutlined, PartitionOutlined,
  WarningOutlined, InboxOutlined,
} from '@ant-design/icons-vue'

function handleError(err: any) { console.warn('[WMS波次]', err) }
...
const waveTypeMap: Record<number, string> = {
  0: '按单拣货', 1: '批量拣货', 2: '按路线',
}

const columns = [
  { title: '波次编号', dataIndex: 'waveNo', width: 150 },
  { title: '仓库', dataIndex: 'warehouseName', width: 120 },
  { title: '波次类型', dataIndex: 'waveType', width: 100 },
...
const searchFields: SearchField[] = [
  { name: 'waveNo', label: '波次编号', type: 'input', placeholder: '请输入波次编号' },
  { name: 'waveType', label: '波次类型', type: 'select', placeholder: '请选择波次类型', options: [
    { label: '按单拣货', value: 0 }, { label: '批量拣货', value: 1 }, { label: '按路线', value: 2 },
  ]},
  { name: 'status', label: '状态', type: 'select', placeholder: '请选择状态', options: [
    { label: '待处理', value: 0 }, { label: '处理中', value: 1 }, { label: '已完成', value: 2 }, { label: '已取消', value: 3 }, { label: '异常', value: 4 },
  ]},
]

const wms = useWmsTable({
  fetchFn: pickApi.wavePage as any,
  deleteFn: pickApi.waveRemove,
  defaultPageSize: 20,
  refreshInterval: 30,
...
const modalTitle = computed(() => (isEdit.value ? '编辑波次' : '新增波次'))
const formRef = ref<FormInstance>()
const formState = reactive<Record<string, any>>({
  id: 0, waveNo: '', warehouseName: '', waveType: 0, totalQty: 0, remark: '',
})
...
function handleAdd() {
  ...
  Object.assign(formState, { id: 0, waveNo: '', warehouseName: '', waveType: 0, totalQty: 0, remark: '' })
  modalVisible.value = true
}

function handleEdit(record: WmsPickWave) {
  isEdit.value = true
...
async function handleModalOk() {
  ...
  try {
    ...
    if (isEdit.value) {
      await pickApi.waveUpdate(payload)
      message.success('更新成功')
    } else {
      await pickApi.waveSave(payload)
...
function handleView(record: WmsPickWave) {
  message.info(`查看波次: ${record.waveNo}`)
}

// ── 工作流操作 ──
async function handleCreateTasks(record: WmsPickWave) {
  Modal.confirm({
    title: '生成拣货任务',
    content: `确定为波次"${record.waveNo}"生成拣货任务吗？`,
    onOk: async () => {
      try {
        await pickApi.waveCreate({ waveId: record.id } as any)
        message.success('拣货任务已生成')
        wms.fetchData()
      } catch (err: any) {
