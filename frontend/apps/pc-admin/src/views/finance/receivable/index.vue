            ...
            <template #action="{ record }">
              <a-space :size="0" class="action-cell-inner">
                <PrintButton
                  ...
                  tooltip="打印"
                />
                <a-tooltip :title="record.status === 'written_off' || record.status === 'bad_debt' ? '' : '核销'">
                  <a-button type="link" size="small" v-permission="'finance:receivable:writeoff'" :disabled="record.status === 'written_off' || record.status === 'bad_debt'" @click="handleWriteOff(record)">
                ...
                <a-dropdown trigger="click">
                  ...
                  <template #overlay>
                    <a-menu @click="({ key }: any) => handleActionMenuClick(key as string, record)">
                      <a-menu-item key="badDebt" v-permission="'finance:receivable:baddebt'" :disabled="record.status === 'written_off' || record.status === 'bad_debt'" danger>
                        <DeleteOutlined /> 坏账标记
                      </a-menu-item>
                    </a-menu>
                  </template>
...
const overdueAmount = computed(() => {
  return tableData.value
    .filter(r => isOverdue(r.dueDate) && r.remainingAmount > 0)
  ...
  try {
    ...
    if (res.data) {
      ...
      if (res.totalAmount !== undefined) {
        ...
        stats.writtenOffAmount = res.totalPaidAmount || 0
        stats.balanceAmount = res.totalRemainingAmount || 0
      } else {
...
.finance-receivable-page > :deep(.vxe-table-list-container) {
  flex: 1;
  min-height: 0;
}
